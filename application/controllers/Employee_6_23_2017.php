...
class Employee extends MY_Controller {
	...
	if($this->session->userdata('user_type_id') == 10){ // hr
		$person_type_id = 1;
	}
	else if($this->session->userdata('user_type_id') == 10){ // dietitian
		$person_type_id = 6;
	}
	$person_type_id = decode_string($this->uri->segment(3));
	
        /* Updated by Paul Alcabasa
	   June 23, 20217 2:32 PM
           Bug found for HR User
	   Note : Check if operation is successful before returning user to datagrid of employees
	*/
	if($this->session->flashdata('success_flag')) {
    ...
    public function ajax_get_employees_with_credit_by_department(){
        ...
        $person_type_id = 1; // mdi employees
        if($department_id != 0){
            $employees_list = $this->person_model->get_persons_with_credit_by_dept($department_id);
        }
        else {
            $employees_list = $this->person_model->get_persons_with_credit($person_type_id);
        }
        $ctr = 1;
        foreach($employees_list as $emp){
