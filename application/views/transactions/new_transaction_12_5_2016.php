	              	...
	              	<div class="row">
			        	...
			        	<!-- 	<h5 class="orders-h5">Order Summary</h5>
			        		...
			        		<div class="form-group">
			        			<input type="text" id="temp_transaction_id" value="<?php echo $temp_transaction_no;?>" name="temp_transaction_id"/>
...
function initialize_customer_details(customer_type){
	...
	else if(customer_type == 14 || customer_type == 17 || customer_type == 18){
		$("#employee_details,#patient_details").hide();
		$("#guest_details").show();
	}
...
function addOrder(food_id,food_name,price,table){
	var new_order = "";
	...
	*/
	...
	$("body").on("click",".btn-add-to-order",function(){
		...
		if($("#temp_transaction_id").val() != ""){
			if(current_qty > 0){
				if(!isExist(food_id)){
					addOrder(food_id,food_name,price,tbl_orders_summary);
					computeTotal();
	...
	$("#txt_amount_tendered").on("blur",function(){
		...
		else if(customer_type == 17 || customer_type == 18){
